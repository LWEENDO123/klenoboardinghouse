...
async function loadBoardingHouse(id, university, studentId) {
  try {
    ...
    // ✅ If university is empty or "default", fall back to current_user’s university
    const uniParam = (!university || university === "default") ? "" : `&university=${university}`;
    const response = await fetch(`${baseUrl}/home/boardinghouse/${id}?student_id=${studentId}${uniParam}`);
